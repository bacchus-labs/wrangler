...
import { AuditEntry } from '../../types/session.js';
 ...
 */
function deriveActiveStep(lastEntry: AuditEntry | undefined): string {
  if (!lastEntry) {
    return 'unknown';
  }
  ...
  } else if (status === 'complete' || status === 'completed' as any) {
 ...
 */
function getCompletedPhases(entries: AuditEntry[]): string[] {
  const completed = new Set<string>();
  for (const entry of entries) {
    if (
      TRACKED_PHASES.has(entry.phase) &&
      (entry.status === 'complete' || (entry.status as string) === 'completed')
    ) {
...
export async function sessionStatusTool(
