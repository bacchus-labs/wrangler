 ...
 * - loadGateMarkdown() - Gate-specific frontmatter parsing
 * - discoverGates() - .md file discovery in directory
...
describe('loadGateMarkdown()', () => {
  it('should parse gate markdown with all gate-specific fields', async () => {
    const filePath = await writeTestFile('gates/security.md', `---
...
`);
    ...
    const result = await loadGateMarkdown(filePath);
    ...
    expect(result.prompt).toContain('security expert');
    expect(result.filePath).toBe(filePath);
  });

  it('should apply defaults for runCondition and enabled', async () => {
    const filePath = await writeTestFile('gates/simple-gate.md', `---
name: basic-gate
description: A basic gate
tools: []
---
...
`);
  ...
  it('should parse gate with enabled=false', async () => {
    const filePath = await writeTestFile('gates/disabled.md', `---
name: disabled-gate
...
`);
  ...
  it('should parse gate with manual runCondition', async () => {
    const filePath = await writeTestFile('gates/manual.md', `---
name: manual-gate
...
`);
    ...
    const result = await loadGateMarkdown(filePath);
    expect(result.runCondition).toBe('manual');
  });

  it('should throw on invalid runCondition value', async () => {
    const filePath = await writeTestFile('gates/bad-condition.md', `---
name: bad
...
`);
    ...
    await expect(loadGateMarkdown(filePath)).rejects.toThrow();
  });

  it('should throw when missing required agent fields', async () => {
    const filePath = await writeTestFile('gates/missing-fields.md', `---
name: incomplete
---
...
`);
  ...
  it('should throw on non-existent file', async () => {
    await expect(
      loadGateMarkdown(path.join(tmpDir, 'gates/nonexistent.md'))
    ).rejects.toThrow();
  });
});
...
describe('discoverGates()', () => {
  ...
  beforeAll(async () => {
    gatesDir = path.join(tmpDir, 'discover-gates');
    await fs.mkdir(gatesDir, { recursive: true });

    // Create .md gate files
    await fs.writeFile(path.join(gatesDir, 'security.md'), '---\nname: sec\n---\n');
    await fs.writeFile(path.join(gatesDir, 'style.md'), '---\nname: sty\n---\n');
    await fs.writeFile(path.join(gatesDir, 'architecture.md'), '---\nname: arch\n---\n');

    // Create non-.md files that should be ignored
    await fs.writeFile(path.join(gatesDir, 'README.txt'), 'Ignore me');
    await fs.writeFile(path.join(gatesDir, 'config.yaml'), 'ignore: true');
    await fs.writeFile(path.join(gatesDir, '.gitkeep'), '');
  });

  it('should discover all .md files in the directory', async () => {
    const files = await discoverGates(gatesDir);
  ...
  it('should return sorted file paths', async () => {
    ...
    expect(names).toEqual(['architecture.md', 'security.md', 'style.md']);
  });
  ...
  it('should not include non-.md files', async () => {
    const files = await discoverGates(gatesDir);
    for (const f of files) {
  ...
  it('should return empty array for empty directory', async () => {
    const emptyDir = path.join(tmpDir, 'empty-gates');
    await fs.mkdir(emptyDir, { recursive: true });
    const files = await discoverGates(emptyDir);
    expect(files).toEqual([]);
  });

  it('should return empty array for directory with no .md files', async () => {
    const noMdDir = path.join(tmpDir, 'no-md-gates');
    await fs.mkdir(noMdDir, { recursive: true });
    await fs.writeFile(path.join(noMdDir, 'config.yaml'), 'x: 1');
    await fs.writeFile(path.join(noMdDir, 'notes.txt'), 'notes');
    const files = await discoverGates(noMdDir);
    expect(files).toEqual([]);
  });

  it('should not discover .md files in subdirectories', async () => {
    const nestedDir = path.join(tmpDir, 'nested-gates');
    const subDir = path.join(nestedDir, 'subdir');
    await fs.mkdir(subDir, { recursive: true });
    await fs.writeFile(path.join(nestedDir, 'top.md'), 'top');
    await fs.writeFile(path.join(subDir, 'nested.md'), 'nested');
    const files = await discoverGates(nestedDir);
    const names = files.map(f => path.basename(f));
