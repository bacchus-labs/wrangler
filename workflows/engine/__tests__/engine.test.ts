...
`);
  ...
  // -----------------------------------------------------------------------
  // Gate-group step execution
  // -----------------------------------------------------------------------
  describe('gate-group step execution', () => {
    it('should discover and execute all gate markdown files', async () => {
      const callLog: Array<{ prompt: string }> = [];
      ...
      await writeGateMarkdown(tmpDir, 'security-review.md', {
        ...gateFrontmatter,
...
`);
      ...
      // Both gates should have been executed
      expect(callLog).toHaveLength(2);
      expect(callLog[0].prompt).toContain('security');
      expect(callLog[1].prompt).toContain('style');
      ...
      expect((review.gateResults as unknown[]).length).toBe(2);
    });

    it('should skip disabled gates', async () => {
      const callLog: Array<{ prompt: string }> = [];

      await writeGateMarkdown(tmpDir, 'enabled-gate.md', {
        name: 'enabled-gate',
        description: 'Enabled',
        tools: [],
        enabled: true,
      ...
      await writeGateMarkdown(tmpDir, 'disabled-gate.md', {
        name: 'disabled-gate',
        description: 'Disabled',
        tools: [],
        enabled: false,
      }, 'Disabled gate prompt.');
...
`);
      ...
      const queryFn = createMockQuery(
        new Map([['default', { assessment: 'approved', issues: [], strengths: [], hasActionableIssues: false }]]),
        callLog as Array<{ prompt: string; options?: QueryOptions['options'] }>
      );
      ...
      // Only the enabled gate should have been called
      expect(callLog).toHaveLength(1);
      expect(callLog[0].prompt).toContain('Enabled gate prompt');
    });

    it('should set empty result when no gates exist', async () => {
      // Create empty gates directory
      await fs.mkdir(path.join(tmpDir, 'empty-gates'), { recursive: true });
...
`);
    ...
    it('should skip gates with changed-files-match condition when no files match', async () => {
      const callLog: Array<{ prompt: string }> = [];

      await writeGateMarkdown(tmpDir, 'ts-review.md', {
        name: 'ts-review',
...
`);
      ...
      // No changed files, so changed-files-match gate should be skipped
      const result = await engine.run('workflow.yaml', '/tmp/spec.md');
...
`);
  ...
  describe('full happy-path workflow', () => {
    it('should execute a complete workflow with all phase types', async () => {
      ...
      // Create gate files
      await writeGateMarkdown(tmpDir, 'quality-gate.md', {
        name: 'quality-gate',
...
`);
      ...
      // analyze(1) + implement(2 tasks) + review gate(1) + publish(1) = 5 query calls
